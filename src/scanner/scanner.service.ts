...
export class ScannerService implements OnModuleInit {
  ...
  constructor(
    ...
    const apiChainId =
      this.configService.get<string>('API_CHAIN_ID') ||
      'beeab0de00000000000000000000000000000000000000000000000000000000';
    const apiParamAccount =
      this.configService.get<string>('API_PARAM_ACCOUNT') || 'peakd';
    const apiParamCommunity =
      this.configService.get<string>('API_PARAM_COMMUNITY') || 'hive-189727';
    ...
    this.tests = [
      ...
      },
      {
        name: 'db_head_state',
        description:
          'Get the current state of the node (less than 10 mins behind)',
        type: 'fetch',
      ...
      },
      {
        name: 'db_head_age',
        description:
          'Check if the node is in sync with the chain (less than 30 secs behind)',
        type: 'fetch',
        method: 'hive.db_head_state',
        params: {},
        score: 15,
      ...
      },
      {
        ...
        validator: result => {
          ...
          return (
            ...
            result.length === 20 &&
            result[0].created > fiveMinuteAgoString
          );
        },
      },
      {
      ...
      },
      {
        ...
        validator: result => {
          ...
          const isSorted = [...result] // clone array
            ...
            .every(
              (el, i) =>
                el.author === result[i].author &&
                el.permlink === result[i].permlink,
      ...
      },
      {
        ...
        description:
          'Get posts feed for a community and check for pinned/muted posts',
        type: 'fetch',
        method: 'bridge.get_ranked_posts',
        params: {
      ...
      },
      {
        ...
        description:
          'Load transaction history for an account (both activities and wallet transactions)',
        type: 'fetch',
        method: 'call',
        params: [
          'database_api',
          ...
          [
            ...
            -1,
            250,
            ...makeBitMaskFilter([ChainTypes.operations.transfer]),
          ],
        ],
        score: 15,
        debug: false,
      ...
      },
      {
        name: 'transfer',
        description:
          'Try to cast a "transfer" operation (validated as part of the chain consensus)',
        type: 'cast',
        method: 'transfer',
        params: {
          from: beaconAccount,
  ...
  async scan(): Promise<boolean> {
    ...
    try {
      ...
      for (const node of nodes) {
        ...
        for (const test of this.tests) {
          try {
            if (test.type === 'fetch') {
              this.logger.log(
                `Call '${test.name}', params: ${JSON.stringify(
                  test.params,
                )}: ...`,
              );
              ...
              if (success) {
                this.logger.log(
                  `Call '${test.name}', completed in ${elapsed} ms`,
                );
                results.push({
                  name: test.name,
              ...
              if (test.method === 'custom_json') {
                const postingKey = this.configService.get<string>(
                  'BEACON_ACCOUNT_POSTING_KEY',
                );
                if (postingKey) {
                  this.logger.log(
                    `Cast '${test.name}', params: ${JSON.stringify(
                      test.params,
                    )}: ...`,
                  );
                ...
                const activeKey = this.configService.get<string>(
                  'BEACON_ACCOUNT_ACTIVE_KEY',
                );
                if (activeKey) {
                  const params = {
                    ...test.params,
                    memo: this.quotesService.getRandomQuote(),
                  };
                  ...
                  result = await hive.broadcast.sendAsync(
                    { operations: [[test.method, params]], extensions: [] },
                    { active: activeKey.trim() },
                  );
                } else {
              ...
              this.logger.log(
                `Cast '${test.name}', completed in ${elapsed} ms`,
              );
            ...
            this.logger.warn(
              `Call '${test.method}', failed: ${error.toString()}`,
            );
            score -= test.score;
      ...
      this.logger.error(
        `Unexpected error during node scanning: ${error.toString()}`,
      );
      return false;
    } finally {
